...
static void FilterByMaxPrice(List<Room> rooms)
{
    ...
    var result = rooms
        .Where(r => r.IsAvailable && r.PricePerNight <= price)
        .OrderBy(r => r.PricePerNight);
    ...
    foreach (var room in result)
    {
        Console.WriteLine(room.RoomNumber + " - " + room.PricePerNight);
    }
}
